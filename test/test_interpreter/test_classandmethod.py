from src.interpreter.syntax.language import Pass
from src.interpreter.compiler import Compiler
from src.interpreter.syntax.semantics import VObject

def test():
    # テスト用のソースコードを読み込む
    with open("test/test_interpreter/sample/basic/classandmethod.py", "r") as f:
        source_code = f.read()
